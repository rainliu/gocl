...
func main() {
	...
	if err < 0 {
		println("Couldn't set the kernel argument")
		return
	}
	...
	if err < 0 {
		fmt.Printf("Couldn't create the command queue, errcode=%d\n", err)
		return
	}
	...
	err = cl.CLEnqueueNDRangeKernel(queue, kernel, 1, nil, work_units_per_kernel[:],
		nil, 0, nil, nil)
	if err < 0 {
		fmt.Printf("Couldn't enqueue the kernel execution command, errcode=%d\n", err)
		return
	}
	...
	if err < 0 {
		println("Couldn't enqueue the read buffer command")
		return
	}
